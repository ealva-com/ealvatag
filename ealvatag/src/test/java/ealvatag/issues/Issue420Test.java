...
public class Issue420Test extends AbstractTestCase
{
    public void testReadingFieldsThatOnlyExistInID3v1tag() throws Exception
    {
        ...
        assertNull(mp3File.getTag().orNull());
        mp3File.saveMp3();
        mp3File = new MP3File(testFile);
        assertNotNull(mp3File.getTag().or(NullTag.INSTANCE));
        assertEquals(0,mp3File.getTag().or(NullTag.INSTANCE).getFields("TYER").size());
        assertEquals(0,mp3File.getTag().or(NullTag.INSTANCE).getFields(FieldKey.YEAR).size());
