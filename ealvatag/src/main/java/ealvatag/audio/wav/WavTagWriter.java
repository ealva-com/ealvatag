...
public class WavTagWriter {
    ...
    // Logger Object
    private static Logger LOG = LoggerFactory.getLogger(WavTagWriter.class);
        ...
        try {
                ...
                if (fs.isContiguous) {
                    ...
                    if (fs.isAtEnd) {
                        if (fs.isInfoTagFirst) {
                            LOG.info(loggingName + ":Setting new length to:" +
                    ...
                    //Slower
                        ...
                        if (fs.isInfoTagFirst) {
                            final int lengthTagChunk = (int)(existingTag.getEndLocationInFileOfId3Chunk() -
                                    existingTag.getInfoTag().getStartLocationInFile());
            ...
            //Delete Info if exists
                ...
                if (existingInfoTag.getEndLocationInFile() == fc.size()) {
                    LOG.info(loggingName + ":Setting new length to:" + existingInfoTag.getStartLocationInFile());
                    fc.truncate(existingInfoTag.getStartLocationInFile());
                } else {
                ...
                if (isID3TagAtEndOfFileAllowingForPaddingByte(existingTag, fc)) {
                    LOG.info(loggingName + ":Setting new length to:" + existingTag.getStartLocationInFileOfId3Chunk());
                    fc.truncate(existingTag.getStartLocationInFileOfId3Chunk());
                } else {
     ...
     * @throws IOException
     */
    private void deleteInfoTagChunk(final FileChannel fc, final WavTag existingTag, final ChunkHeader chunkHeader)
            throws IOException {
     ...
     * @throws IOException
     */
    private void deleteId3TagChunk(FileChannel fc, final WavTag existingTag, final ChunkHeader chunkHeader)
            throws IOException {
        final int lengthTagChunk = (int)chunkHeader.getSize() + ChunkHeader.CHUNK_HEADER_SIZE;
        deleteTagChunk(fc, (int)existingTag.getEndLocationInFileOfId3Chunk(), lengthTagChunk);
    }
     ...
     * @throws IOException
     */
    private void deleteTagChunk(final FileChannel fc, int endOfExistingChunk, final int lengthTagChunk)
            throws IOException {
    ...
    public ByteBuffer convertInfoChunk(final WavTag tag) throws UnsupportedEncodingException {
        try {
            ...
            for (TagField nextField : fields) {
                ...
                LOG.debug(loggingName + " Writing:" + wii.getCode() + ":" + next.getContent());
                ...
                if (wii == WavInfoIdentifier.TRACKNO) {
                    if (TagOptionSingleton.getInstance().isWriteWavForTwonky()) {
                        ...
                        LOG.debug(loggingName + " Writing:" + WavInfoIdentifier.TWONKY_TRACKNO.getCode() + ":" +
                                          next.getContent());
            ...
            while (ti.hasNext()) {
                ...
                LOG.debug(loggingName + " Writing:" + next.getId() + ":" + next.getContent());
                byte[] contentConvertedToBytes = next.getContent().getBytes(StandardCharsets.UTF_8);
                baos.write(Utils.getSizeLEInt32(contentConvertedToBytes.length));
                baos.write(contentConvertedToBytes);
     ...
     * @throws IOException
     */
    private void deleteExistingMetadataTagsToEndOfFile(final FileChannel fc, final WavTag existingTag)
            throws IOException {
        ...
        if (!Utils.isOddLength(precedingChunk.getEndLocation())) {
            LOG.error(loggingName + " Truncating corrupted metadata tags from:" +
