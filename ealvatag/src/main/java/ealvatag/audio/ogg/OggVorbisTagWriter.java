...
public class OggVorbisTagWriter {
    private static Logger LOG = LoggerFactory.getLogger(OggVorbisTagWriter.class);
    ...
    public void write(TagFieldContainer tag, RandomAccessFile raf, RandomAccessFile rafTemp)
        ...
        LOG.debug("Read 2nd Page:comment and setup and possibly audio:Header finishes at file position:" +
        ...
        LOG.debug("Old 2nd Page no of packets: " + secondPageHeader.getPacketList().size());
        LOG.debug("Old 2nd Page size: " + secondPageHeader.getPageLength());
        LOG.debug("Old last packet incomplete: " + secondPageHeader.isLastPacketIncomplete());
        LOG.debug("Setup Header Size: " + vorbisHeaderSizes.getSetupHeaderSize());
        LOG.debug("Extra Packets: " + vorbisHeaderSizes.getExtraPacketList().size());
        LOG.debug("Extra Packet Data Size: " + vorbisHeaderSizes.getExtraPacketDataSize());
    ...
    private void replaceSecondPageAndRenumberPageSeqs(OggVorbisTagReader.OggVorbisHeaderSizes originalHeaderSizes,
        ...
        byte[] setupHeaderData =
                reader.convertToVorbisSetupHeaderPacketAndAdditionalPackets(originalHeaderSizes
                                                                            ...
                                                                            raf);
        LOG.debug(setupHeaderData.length + ":" + secondPageBuffer.position() + ":" + secondPageBuffer.capacity());
    ...
    private void replacePagesAndRenumberPageSeqs(OggVorbisTagReader.OggVorbisHeaderSizes originalHeaderSizes,
        ...
        LOG.debug("Comment requires:" + noOfCompletePagesNeededForComment + " complete pages");
        ...
        if (!isCommentAndSetupHeaderFitsOnASinglePage(lastPageCommentPacketSize,
            ...
            {
                byte[] segmentTable = this.createSegmentTable(originalHeaderSizes.getSetupHeaderSize(),
                                                              originalHeaderSizes.getExtraPacketList());
                int pageHeaderLength = OggPageHeader.OGG_PAGE_HEADER_FIXED_LENGTH + segmentTable.length;
                byte[] setupHeaderData =
                        reader.convertToVorbisSetupHeaderPacketAndAdditionalPackets(originalHeaderSizes
                ...
                LOG.debug("Writing Setup Header and packets Page " + pageSequence + " to file");
    ...
    private byte[] createSegmentTable(int newCommentLength,
        ...
        LOG.debug("Create SegmentTable CommentLength:" + newCommentLength + ":SetupHeaderLength:" + setupHeaderLength);
        ...
        try {
            ...
            if (extraPackets.size() > 0) {
                ...
                LOG.debug("Creating segments for " + extraPackets.size() + " packets");
    ...
    private byte[] createSegments(int length, boolean quitStream) {
        LOG.debug("Create Segments for length:" + length + ":QuitStream:" + quitStream);
    ...
    private boolean isCommentAndSetupHeaderFitsOnASinglePage(int commentLength,
        ...
        LOG.debug("Require:" + totalDataSize + " segments for comment plus setup");
        ...
        LOG.debug("Total No Of Segment If New Comment And Header Put On One Page:" + totalDataSize);
